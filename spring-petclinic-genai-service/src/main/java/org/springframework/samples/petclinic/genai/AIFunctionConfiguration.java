...
/**
 * This class defines the @Bean functions that the LLM provider will invoke when it requires more
 * Information on a given topic. The currently available functions enable the LLM to get the list of
 * owners and their pets, get information about the veterinarians, and add a pet to an owner.
 *
 * @author Oded Shopen
 */
@Configuration
class AIFunctionConfiguration {
  ...
  public Function<OwnerRequest, OwnerResponse> addOwnerToPetclinic(
      AIDataProvider petclinicAiDataProvider) {
  ...
  @Bean
  @Description(
      "Add a pet with the specified petTypeId, "
          ...
          + "5 - bird"
          + "6 - hamster")
  public Function<AddPetRequest, AddedPetResponse> addPetToOwner(
      AIDataProvider petclinicAiProvider) {
